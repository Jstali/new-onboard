...
const ADPPayrollForm = ({ employeeId, isReadOnly = false }) => {
  ...
  const formSections = {
    ...
    "LWF Information": [
      ...
      "LWF Relationship",
      "LWFID",
      "Professional tax group Description",
      "PF Computational Group",
    ],
    "Contact Information": ["Mobile Number", "Phone Number1", "Phone Number2"],
    "Address Information": [
      "Address1",
      "Address2",
      "Address3",
    ...
    "PF/ESI Information": [
      "PF account no",
      "ESI account no",
      "ESI Above Wage Limit",
      "UAN",
      "Branch description",
      "Enrollment ID",
      "Manager Employee ID",
      "Tax regime",
      "Already Member In PF",
      "Already Member In Pension",
      "Withdrawn PFand Pension",
    ...
    "Disability Information": ["DisabilityStatus", "TypeOfDisability"],
    "Custom Groups": [
      "Custom Group 1",
      "Custom Group 2",
      "Custom Group 3",
    ...
    "Additional Information": [
      ...
      "Driving Licence ValidDate",
      "PRAN Number",
      "Rehire",
      "Old Employee ID",
      "IsNon Payroll Employee",
      "Category Name",
      "Custom Master Name",
      "Custom Master Name2",
      "Custom Master Name3",
    ...
    "Eligibility Information": [
      "Ot Eligibility",
      "Auto Shift Eligibility",
      "Mobile User",
      "Web Punch",
      "Attendance Exception Eligibility",
  ...
  const handleSave = async (isDraftSave = true) => {
    ...
    try {
      ...
      const response = await axios.post(
        `/adp-payroll/employee/${employeeId}/${endpoint}`,
        formData
      );
  ...
  const handleFinalize = async () => {
    ...
    try {
      const response = await axios.put(
        `/adp-payroll/employee/${employeeId}/finalize`
      );
      if (response.data.success) {
        toast.success(response.data.message);
        setIsDraft(false);
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      ...
      {!isReadOnly && (
        <div className="flex items-center justify-end space-x-6 mt-8 pt-6 border-t border-gray-200">
          <button
            onClick={() => handleSave(true)}
            disabled={saving}
            className="flex items-center px-6 py-3 bg-yellow-500 text-white rounded-md hover:bg-yellow-600 disabled:opacity-50"
          >
            {saving ? (
          ...
          {isDraft && (
            <button
              ...
              disabled={saving}
              className="flex items-center px-6 py-3 bg-green-500 text-white rounded-md hover:bg-green-600 disabled:opacity-50"
            >
              {saving ? (
                <FaSpinner className="animate-spin mr-2" />
              ) : (
